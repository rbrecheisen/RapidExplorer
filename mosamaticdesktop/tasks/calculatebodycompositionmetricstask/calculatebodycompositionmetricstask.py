...
class CalculateBodyCompositionMetricsTaskTask(Task):
    ...
    def execute(self) -> None:
        ...
        for file in files:
            ...
            if isDicomFile(filePath=file.path()):
                ...
                if segmentationFile:
                    ...
                    LOGGER.info(self.fileOutputMetricsToString(outputMetrics=outputMetrics[fileTuple[0]]))
                else:
                    LOGGER.error(f'Segmentation file {segmentationFile.name()} not found')
        ...
        LOGGER.info(f'Building output fileset: {outputFileSetPath}...')
